...
def test_auth_and_certificate_lifecycle(client):
    ...
    issued = client.post(
        "/v1/pki/certificates",
        headers=headers,
        json={"subject": "test.internal", "days": 30},
    )
    ...
    rotated = client.post(
        f"/v1/pki/certificates/{cert_id}/rotate", headers=headers
    )
    assert rotated.status_code == 200

    revoked = client.post(
        f"/v1/pki/certificates/{rotated.json()['certificate_id']}/revoke",
        headers=headers,
    )
...
def test_real_liboqs_round_trip_if_available(client):
    ...
    created = client.post(
        "/v1/kem/keys", headers=headers, json={"algorithm": algorithm}
    )
    assert created.status_code == 200
    key_id = created.json()["key_id"]

    encapsulated = client.post(
        "/v1/kem/encapsulate", headers=headers, json={"key_id": key_id}
    )
    ...
    decapsulated = client.post(
        "/v1/kem/decapsulate",
        headers=headers,
        json={"key_id": key_id, "ciphertext": encapsulated.json()["ciphertext"]},
    )
    assert decapsulated.status_code == 200
    assert decapsulated.json()["shared_secret"] == encapsulated.json()["shared_secret"]


def test_destroy_removes_decryptable_private_material(client):
    if not kem.capabilities()["available"]:
        pytest.skip("liboqs unavailable")
    ...
    decap = client.post(
        "/v1/kem/decapsulate",
        headers=headers,
        json={"key_id": key_id, "ciphertext": "AA=="},
    )
    assert decap.status_code == 400
